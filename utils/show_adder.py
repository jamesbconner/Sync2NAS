...
def add_show_interactively(show_name, tmdb_id, db: DatabaseInterface, tmdb: TMDBService, anime_tv_path: str, dry_run: bool = False, override_dir: bool = False, llm_service=None, use_llm: bool = False, max_tmdb_results: int = 20):
    ...
    """
    logger.debug(f"utils/show_adder.py::add_show_interactively - Called with show_name={show_name}, tmdb_id={tmdb_id}, override_dir={override_dir}, dry_run={dry_run}, use_llm={use_llm}, max_tmdb_results={max_tmdb_results}")
    try:
        if tmdb_id:
            logger.info("utils/show_adder.py::add_show_interactively - Using tmdb_id branch")
            details = tmdb.get_show_details(tmdb_id)
            if not details or "info" not in details:
                logger.error("utils/show_adder.py::add_show_interactively - tmdb_id Branch - Could not retrieve show details for TMDB ID")
            ...
            logger.info("utils/show_adder.py::add_show_interactively - Using LLM branch")
            ...
            if not results or not results.get("results"):
                logger.error(f"utils/show_adder.py::add_show_interactively - LLM Branch - No results found for show name: {show_name}")
            ...
            if not detailed_results:
                logger.error(f"utils/show_adder.py::add_show_interactively - LLM Branch - No detailed TMDB results for show name: {show_name}")
                raise ValueError(f"LLM Branch - No detailed TMDB results for show name: {show_name}")
            logger.debug("utils/show_adder.py::add_show_interactively - LLM Branch - Calling llm_service.suggest_show_name")

            # Use LLM to select best match and English name
            llm_response = llm_service.suggest_show_name(show_name, detailed_results)
            logger.debug(f"utils/show_adder.py::add_show_interactively - LLM Branch - LLM response: {llm_response}")
            ...
            if not details or "info" not in details:
                logger.error(f"utils/show_adder.py::add_show_interactively - LLM Branch - Failed to retrieve full details for TMDB ID {tmdb_id}")
                raise ValueError(f"LLM Branch - Failed to retrieve full details for TMDB ID {tmdb_id}")
            
        elif show_name:
            logger.info("utils/show_adder.py::add_show_interactively - Using show_name branch")
            results = tmdb.search_show(show_name)
            if not results or not results.get("results"):
                logger.error(f"utils/show_adder.py::add_show_interactively - show_name Branch - No results found for show name: {show_name}")
                raise ValueError(f"show_name Branch - No results found for show name: {show_name}")
            first_result = results["results"][0]
            tmdb_id = first_result["id"]
            details = tmdb.get_show_details(tmdb_id)
            if not details or "info" not in details:
                logger.error(f"utils/show_adder.py::add_show_interactively - show_name Branch - Failed to retrieve full details for TMDB ID {tmdb_id}")
                raise ValueError(f"show_name Branch - Failed to retrieve full details for TMDB ID {tmdb_id}")
            sys_name = show_name
        else:
            logger.error("utils/show_adder.py::add_show_interactively - Neither show_name nor tmdb_id provided")
            raise ValueError("Neither show_name nor tmdb_id provided")

        if override_dir:
            sys_path = os.path.join(anime_tv_path, sanitize_filename(show_name))
            logger.debug(f"utils/show_adder.py::add_show_interactively - override_dir Branch - Using provided show_name directly for the folder name: {sys_path}")
            sys_name = show_name
            logger.debug(f"utils/show_adder.py::add_show_interactively - override_dir Branch - Using provided show_name directly for the sys_name: {sys_name}")
        else:
            sys_path = os.path.join(anime_tv_path, sanitize_filename(sys_name))
            logger.debug(f"utils/show_adder.py::add_show_interactively - Using sanitized derived sys_name for the folder name: {sys_path}")
        ...
        if dry_run:
            logger.info(f"utils/show_adder.py::add_show_interactively - [DRY RUN] Would create directory: {sys_path}")
            logger.info(f"utils/show_adder.py::add_show_interactively - [DRY RUN] Would insert show: {show.tmdb_name}")
            logger.info(f"utils/show_adder.py::add_show_interactively - [DRY RUN] Would insert {len(episodes)} episodes")
        else:
            os.makedirs(sys_path, exist_ok=True)
            db.add_show(show)
            db.add_episodes(episodes)
            logger.info(f"utils/show_adder.py::add_show_interactively - ✅ Created directory and added show '{show.tmdb_name}' with {len(episodes)} episodes")
        ...
        logger.error(f"utils/show_adder.py::add_show_interactively - Exception: {e}", exc_info=True)
        raise
