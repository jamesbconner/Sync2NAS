...
class DBService:
    """Manages the persistence layer for TV shows and episodes using SQLite.
    
    This class handles all database operations including:
    - Creating and maintaining the database schema
    - Adding and retrieving shows and episodes
    - Updating show information from TMDB
    """
    ...
    def __str__(self):
        ...
        return f"DatabaseService(db_file={self.db_file})"
    ...
    def _register_sqlite_datetime_adapters(self):
        ...
        This enables proper storage and retrieval of datetime objects in SQLite.
        """
        try:
    ...
    def _check_database_path(self):
        ...
        Returns:
            bool: True if the database path exists or was created successfully
        """
    ...
    def _initialize_database(self):
        ...
        Creates tables for:
        - tv_shows: Main show information
        - episodes: Episode information
        - downloaded_files: Local file metadata
        - sftp_temp_files: SFTP file metadata
        """
        ...
        with self._connection() as conn:
            ...
            logger.debug(f"Database initialized with tables: {conn.execute("SELECT name FROM sqlite_master WHERE type = 'table'").fetchall()}")
 
    def initialize(self):
        self._initialize_database()
  
    def clear_sftp_temp_files(self):
    ...
    def insert_sftp_temp_files(self, entries: List[dict]):
    ...
    def get_sftp_diffs(self) -> List[dict]:
        """
        Return a list of files present in sftp_temp_files but not in downloaded_files.

        The comparison includes name, size, modified_time, path, and is_dir.
        """
        with self._connection() as conn:
    ...
    def copy_sftp_temp_to_downloaded(self):
        """
        Copy all entries from sftp_temp_files into downloaded_files.
        Existing entries in downloaded_files are not removed.
        """
        with self._connection() as conn:
            conn.execute('''
                INSERT INTO downloaded_files (name, size, modified_time, path, fetched_at, is_dir)
                SELECT name, size, modified_time, path, fetched_at, is_dir FROM sftp_temp_files ORDER BY modified_time ASC
            ''')
            conn.commit()
            logger.info("Copied sftp_temp_files to downloaded_files.")

    def add_inventory_files(self, files: List[dict]) -> None:
        """
    ...
    def add_downloaded_files(self, files: List[dict]) -> None:
        """
        Insert a list of downloaded file metadata into the downloaded_files table.

        Each dictionary in `files` should contain:
            - name
            - size
            - modified_time
            - path
    ...
    def show_exists(self, name: str) -> bool:
        ...
        with self._connection() as conn:
            ...
            for row in cursor.fetchall():
                ...
                # Sometimes show names are comma separated, so we need to check all variations
                ...
                # If the name_set.intersection(match_candidates) is not empty, then the show exists
                # The name_set must also have at least two elements to avoid false positives
    ...
    def get_show_by_sys_name(self, sys_name: str) -> Optional[dict]:
    ...
    def get_show_by_alias(self, alias: str) -> Optional[dict]:
        """Retrieve a show record by alias."""
        with self._connection() as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.execute("SELECT * FROM tv_shows WHERE LOWER(tmdb_aliases) LIKE ?", (f"%{alias.lower()}%",))
            return cursor.fetchone()

    def get_show_by_name_or_alias(self, name: str) -> Optional[dict]:
        with self._connection() as conn:
            conn.row_factory = sqlite3.Row
    ...
    def get_episodes_by_tmdb_id(self, tmdb_id: int) -> List[dict]:
    ...
    def get_episodes_by_show_name(self, show_name: str) -> List[dict]:
    ...
    def get_episode_by_absolute_number(self, tmdb_id: int, abs_episode: int) -> Optional[dict]:
    ...
    def delete_show_and_episodes(self, tmdb_id: int):
